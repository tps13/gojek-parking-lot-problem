...
public class ParkingLot {
	...
	public void parkCar(Car car) {
		if (noOfParkingSlots == 0) {
			...
			if (colorCarMap.containsKey(car.getColour())) {
				List<String> regNoList = colorCarMap.get(car.getColour());
				colorCarMap.remove(car.getColour());
				regNoList.add(car.getRegistrationNumber());
				colorCarMap.put(car.getColour(), regNoList);
			} else {
	...
	public void leaveSlot(Integer slotNo) {
		if (noOfParkingSlots == 0) {
			...
			if (carToLeave != null) {
				...
				List<String> regNoList = colorCarMap.get(carToLeave.getColour());
